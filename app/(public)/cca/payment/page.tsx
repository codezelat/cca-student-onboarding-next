 ...
 * Built with premium glassmorphism aesthetics aligned with /register
 * Implements progressive disclosure UX for payment slip updates
 */
export default function PaymentUpdatePage() {
    ...
    const identifierLabel = studentType === "local" ? "National ID (NIC) Number" : "Passport Number";
    const identifierPlaceholder = studentType === "local" ? "E.g., 981234567V" : "E.g., N1234567";
    ...
    const handleFetchDetails = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`/api/registrations/lookup?type=${studentType}&identifier=${encodeURIComponent(identifier)}`);
            const result = await response.json();

            if (response.ok && result.success && result.data) {
                setStudentDetails(result.data);
            } else {
                setFetchError(result.error || "No registered student found with this detail. Please check and try again.");
            }
        } catch (error) {
            console.error("Failed to fetch student details:", error);
            setFetchError("An error occurred while fetching details. Please try again later.");
        } finally {
            setIsFetching(false);
        }
    };
            ...
            <div className="max-w-3xl mx-auto relative z-10">
                ...
                {submitStatus === "success" ? (
                    <div className="bg-white/60 backdrop-blur-xl border border-white/80 shadow-2xl rounded-3xl p-8 sm:p-12 text-center text-gray-800 transform transition-all duration-500 hover:shadow-primary-500/10">
                        ...
                        <h2 className="text-3xl font-bold mb-4 text-gray-800">Payment Slip Submitted!</h2>
                        <p className="text-lg text-gray-600 mb-8 max-w-md mx-auto">
                            Thank you. Our finance team will review your payment slip and update your verified balance shortly.
                        </p>
                        <button
                    ...
                    <div className="space-y-6 sm:space-y-8">
                        ...
                        <div className="bg-white/60 backdrop-blur-xl border border-white/80 shadow-xl rounded-2xl p-6 sm:p-8 transition-all">
                            <div className="flex items-center gap-4 mb-6">
                                ...
                                <div>
                                    <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Student Identity</h2>
                                    <p className="text-sm text-gray-600">Enter your details to retrieve your fee structure</p>
                                </div>
                            </div>
                            ...
                            <form onSubmit={handleFetchDetails} className="space-y-6">
                                ...
                                <div className="flex bg-gray-100/80 p-1.5 rounded-xl">
                                    <button
                                        type="button"
                                        onClick={() => { setStudentType("local"); setIdentifier(""); setStudentDetails(null); }}
                                        className={`flex-1 py-2.5 text-sm sm:text-base font-semibold rounded-lg transition-all duration-300 ${studentType === "local"
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => { setStudentType("international"); setIdentifier(""); setStudentDetails(null); }}
                                        className={`flex-1 py-2.5 text-sm sm:text-base font-semibold rounded-lg transition-all duration-300 ${studentType === "international"
                                            ? "bg-white text-primary-600 shadow-sm"
                                            : "text-gray-500 hover:text-gray-700 hover:bg-gray-200/50"
                                            }`}
                                    >
                                ...
                                <div>
                                    ...
                                    />
                                    {fetchError && (
                                        <p className="mt-2 text-sm text-red-500 animate-pulse">{fetchError}</p>
                                    )}
                                </div>
                                ...
                                {!studentDetails && (
                                    <button
                                        ...
                                        className="w-full py-3.5 rounded-xl bg-gray-900 hover:bg-gray-800 text-white font-semibold transition-all shadow-md hover:shadow-lg disabled:opacity-70 flex justify-center items-center gap-2"
                                    >
                                        {isFetching ? (
                                            <>
                            ...
                            <div className="bg-white/60 backdrop-blur-xl border border-white/80 shadow-xl rounded-2xl p-6 sm:p-8 animate-fade-in-up">
                                <div className="flex items-center justify-between mb-6">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Hi, {studentDetails.firstName} </h2>
                                            <p className="text-sm text-gray-600 truncate max-w-[200px] sm:max-w-md">Financial Overview for {studentDetails.fullName}</p>
                                        </div>
                                    </div>
                                ...
                                <div className="mb-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div className="p-4 bg-gray-50/80 rounded-xl border border-gray-100">
                                        ...
                                        <p className="font-semibold text-gray-800 truncate" title={studentDetails.fullName}>{studentDetails.fullName}</p>
                                    </div>
                                    <div className="p-4 bg-gray-50/80 rounded-xl border border-gray-100">
                                        <p className="text-sm text-gray-500 mb-1">Enrolled Program</p>
                                        <p className="font-semibold text-gray-800 truncate" title={studentDetails.programName}>{studentDetails.programName}</p>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                                    <div className="bg-white/80 rounded-xl p-4 border border-gray-100 shadow-sm">
                                        ...
                                        <p className="text-lg font-bold text-gray-800">{formatCurrency(studentDetails.fullAmount)}</p>
                                    </div>
                                    <div className="bg-green-50/80 rounded-xl p-4 border border-green-100 shadow-sm">
                                        ...
                                        <p className="text-lg font-bold text-green-700">{formatCurrency(studentDetails.paidAmount)}</p>
                                    </div>
                                    <div className="bg-primary-50/80 rounded-xl p-4 border border-primary-200 shadow-sm relative overflow-hidden">
                                        ...
                                        <p className="text-sm text-primary-600 mb-1 relative z-10">Balance Due</p>
                                        <p className="text-lg font-bold text-primary-700 relative z-10">{formatCurrency(studentDetails.balanceDue)}</p>
                                    </div>
                                </div>

                                {/* Upload Zone */}
                                <div className="border-t border-gray-100 pt-8 mt-4">
                                    <h3 className="text-lg font-bold text-gray-800 mb-4">Upload New Payment Slip</h3>

                                    <div
                                        onClick={() => !isSubmitting && fileInputRef.current?.click()}
                                        className={`border-2 border-dashed rounded-2xl p-8 text-center transition-all cursor-pointer ${paymentFile
                                    ...
                                    >
                                        ...
                                        {!paymentFile ? (
                                            <div className="flex flex-col items-center">
                                                ...
                                                <p className="font-semibold text-gray-700">Click to upload payment slip</p>
                                            ...
                                            <div className="flex flex-col items-center">
                                                ...
                                                <p className="font-semibold text-gray-800">{paymentFile.name}</p>
                                                <p className="text-sm text-gray-500 mt-1">{(paymentFile.size / (1024 * 1024)).toFixed(2)} MB</p>
                                    ...
                                    >
                                        ...
                                        {isSubmitting && (
                                            <div className="absolute inset-0 bg-black/20 origin-left transition-all duration-300 pointer-events-none" style={{ width: `${uploadProgress}%` }}></div>
                                        )}
                                    </button>
                                </div>
                            </div>
